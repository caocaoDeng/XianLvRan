...
import type { Size, GenderValue, MediaTypeValue } from './interface'

export const getSizeForUrl = (url: string): Size => {
  const REG = /w=(?<w>.+)&h=(?<h>.+)/ // 捕获组
  const defaultSize = { w: '418', h: '418' }
  const size = REG.exec(url)?.groups || defaultSize
  const sizeEntries = Object.entries(size).map(([k, v]) => [k, +v])
